...
import { makeGetRequest } from "../src/request/BrowserRequest";
...
describe("browserRequest", () => {
  describe("makeGetRequest", () => {
    ...
    it("makes a GET request to the argument URL", async () => {
      const req = makeGetRequest("http://localhost:5000/datafiles/123.json", {});

      expect(xhrs.length).toBe(1);
      const xhr = xhrs[0];
      const { url, method } = xhr;
      expect({ url, method }).toEqual({
        url: "http://localhost:5000/datafiles/123.json",
        method: "GET",
      });
    ...
    it("includes headers from the headers argument in the request", async () => {
      const req = makeGetRequest("http://localhost:5000/dataifles/123.json", {
    ...
    it("sets a timeout on the request object", () => {
      const onCreateMock = jest.fn<any, any>();
      mockXHR.onCreate = onCreateMock;
      makeGetRequest("http://localhost:5000/datafiles/123.json", {});
