import HttpPollingDatafileManager from "../src/manager/HttpPollingDatafileManager";
import { Headers, AbortableRequest, Response } from "../src/@types/http";
import { DatafileManagerConfig } from "../src/@types/datafileManager";
import { advanceTimersByTime, getTimerCount } from "./testUtils";
import PersistentKeyValueCache from "../src/@types/persistentKeyValueCache";
    ...
    describe("when constructed with sdkKey and autoUpdate: true", () => {
        ...
        describe("started state", () => {
            it("passes the default datafile URL to the makeGetRequest method", async () => {
                ...
                expect(makeGetRequestSpy.mock.calls[0][0]).toBe("http://localhost:5000/datafiles/123.json");
            ...
            describe("live updates", () => {
                ...
                describe("backoff", () => {
                    it("uses the delay from the backoff controller getDelay method when greater than updateInterval", async () => {
                        const BackoffControllerMock = (BackoffController as unknown) as jest.Mock<
                            BackoffController,
                            []
                        >;
