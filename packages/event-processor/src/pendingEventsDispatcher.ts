...
import { EventDispatcher, EventRequest, EventDispatcherCallback } from "./@types/eventDispatcher";
...
export class PendingEventsDispatcher implements EventDispatcher {
  ...
  dispatchEvent(request: EventRequest, callback: EventDispatcherCallback): void {
    this.send(
      {
        uuid: generateUUID(),
