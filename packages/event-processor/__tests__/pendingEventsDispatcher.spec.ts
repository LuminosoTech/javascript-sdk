...
import { EventDispatcher, EventRequest } from "../src/@types/eventDispatcher";
import { Event } from "../src/event/buildEvent";
...
describe("LocalStoragePendingEventsDispatcher", () => {
  ...
  it("should properly send the events to the passed in eventDispatcher, when callback statusCode=200", () => {
    ...
    const eventV1Request: EventRequest = {
      url: "http://cdn.com",
      httpVerb: "POST",
      params: ({ id: "event" } as unknown) as Event,
    };
    ...
    const internalDispatchCall = ((originalEventDispatcher.dispatchEvent as unknown) as jest.Mock).mock.calls[0];
  ...
  describe("sendPendingEvents", () => {
    ...
    describe("when there are multiple pending events in the store", () => {
      it("should dispatch all of the pending events, and remove them from store", () => {
        ...
        const callback = jest.fn();
        const eventV1Request1: EventRequest = {
          url: "http://cdn.com",
          httpVerb: "POST",
          params: ({ id: "event1" } as unknown) as Event,
        };

        const eventV1Request2: EventRequest = {
          url: "http://cdn.com",
          httpVerb: "POST",
          params: ({ id: "event2" } as unknown) as Event,
        };
        ...
        // manually invoke original eventDispatcher callback
        const internalDispatchCalls = ((originalEventDispatcher.dispatchEvent as unknown) as jest.Mock).mock.calls;
